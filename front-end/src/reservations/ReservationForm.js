...
export default function ReservationForm({ reservation_id }) {
  ...
  function handleChange({ target }) {
    ...
    // checks reservation date is not on a Tuesday nor past date
    if (name === "reservation_date") {
      const date = new Date(`${value} EST`);
      ...
      if (date.getUTCDay() === 2 && reservation < now) {
        ...
        setReservationError(["The restaurant is closed on Tuesday."]);
      } else if (reservation < now) {
        setReservationError(["Reservation must be in the future."]);
      } else {
        setReservationError([]);
      }
    }
  ...
  function submitHandler(event) {
    ...
    if (reservation_id) {
      async function updateData() {
        try {
          setReservationError([]);
          await updateReservation(form, abortController.signal);
          history.push(`/dashboard?date=${form.reservation_date}`);
        } catch (error) {
          setReservationError([...reservationError, error.message]);
        }
